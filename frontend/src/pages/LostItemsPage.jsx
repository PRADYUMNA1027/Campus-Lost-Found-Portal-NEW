...
const LostItemsPage = () => {
  ...
  const fetchLostItems = async () => {
    ...
    try {
      // Call GET /api/items?status=Lost&category=...&location=...&query=...
      ...
      setError('Something went wrong loading lost items. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Belongings currently reported missing on campus. Submit a report if you found one.
          </p>
        </div>
      ...
      {/* Error State */}
      {error && <ErrorMessage message={error} onRetry={fetchLostItems} />}

      {/* Results Count & Grid */}
      {loading ? (
        <LoadingSpinner message="Fetching live lost items directory..." />
      ) : items.length > 0 ? (
        <div className="space-y-4">
          <div className="text-xs font-semibold text-slate-500">
            Showing {items.length} lost item{items.length !== 1 ? 's' : ''}
          </div>
        ...
        <EmptyState
          ...
          message="Try adjusting your category filter, location choice, or search keywords."
        />
      )}
    </div>
  );
